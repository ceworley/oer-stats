...
sig2 = signif(mu1*runif(1,0.03,0.35),3)
...
se = signif(sqrt(s1^2/n1+s2^2/n2) ,3)
...
if(abs(abs(t)-tstar)>0.01 & abs(abs(t)-tstar)<0.3)
{
    break
}
}
...
for (i in 1:n1)
{
lines =  paste0(lines,paste(c(sam1[i],sam2[i]),collapse=" | "),"\n",sep="")
}
```
...
You are interested in whether a treatment causes an effect on a continuously measurable attribute. You use a treatment group with `r n1` cases and a control group with `r n2` cases.  You decide to run a hypothesis test with a significance level of `r alpha`. Your data is below. Please use `r df` for the degrees of freedom (calculated with the Welch-Satterthwaite equation).
...
$$t^{\star} = `r tstar`$$
Determine the sample statistics. Use a calculator!
$$
\begin{aligned}
\bar{x}_1 &= `r xbar1` \\
s_1 &= `r s1` \\
