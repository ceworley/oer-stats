...
### define tree from openintro (https://github.com/OpenIntroStat/openintro-r-package/blob/master/R/treeDiag.R)
...
#          scientific = FALSE
#        )
        sol <- format(signif(p1[i] * p2[[i]][j], digits),
          scientific = FALSE
        )
        if (showWork) {
...
############ DATA GENERATION
images = c("bike","cat","dog","flower","gem","horn",
           "kite","llama","pig","ring","shovel","tree","wheel")
colors = c("red","orange","yellow","green","blue","indigo",
           "violet","white","black","teal","pink","gray")
image = sample(images,1)
color = sample(colors,1)
pi = signif(runif(1,0.1,0.5),3)
while(T)
{
pcgi = signif(runif(1,0.1,0.9),3)
pcgni = signif(runif(1,0.1,0.9),3)
if(abs(pcgi-pcgni)>0.1 & abs(pcgi-pi)>0.1 & abs(pi-pcgni)>0.1)
{
    break
}
}
type = sample(1:4,1)
wrd1 = switch(type,"","","not ","not ")
wrd2 = switch(type,"","not ","","not ")

pnum = switch(type,pi*pcgi,pi*(1-pcgi),(1-pi)*pcgni,(1-pi)*(1-pcgni))
poth = switch(type,(1-pi)*pcgni,(1-pi)*(1-pcgni),pi*pcgi,pi*(1-pcgi))
p3 = signif(pnum,3)
p4 = signif(poth,3)
p5 = signif(p3/(p3+p4),3)
```

Question
========

In a deck of strange cards, each card has an image and a color. The chance of drawing a `r image` is `r pi*100`%. If a `r image` is drawn, there is a `r pcgi*100`% chance that it is `r color`. If a card that is not a `r image` is drawn, there is a `r pcgni*100`% chance that it is `r color`. 

Now, someone draws a random card and reveals it is `r wrd2` `r color`. What is the chance the card is `r wrd1` a `r image`?


Solution
========

I'd recommend making a tree. Remember, on the first branch, we put simple probabilities. On the second branches we put conditional probabilites. The results (products) are joint probabilities.

```{r tree, echo = FALSE, results = "hide", message=FALSE,fig.width=6} 
treeDiag(c('image', 'color'), out1 = c(image,paste("not ",image,sep="")), 
               out2 = c(color,paste("not ",color,sep="")), 
               c(pi, 1-pi), 
               textwd = 0.3,
               list(c(pcgi, 1-pcgi),c(pcgni, 1-pcgni)), 
               showWork = TRUE, 
               solwd=0.5,
               digits=3)
```
...
$$P(\text{"`r wrd1``r image`" given "`r wrd2``r color`"}) = \frac{`r p3`}{`r p3`+`r p4`} = `r p5` $$


Meta-information
============
extype: string
exsolution: $P(\text{"`r wrd1``r image`" given "`r wrd2``r color`"}) = `r p5` $
exname: cards reverse bayesian inference
extol: 0.01
