...
p1 = switch(type,pi,pi,1-pi,1-pi)
p2 = switch(type,pcgi,1-pcgi,pcgni,1-pcgni)
p2b = switch(type,pcgni,1-pcgni,pcgi,1-pcgi)
p3 = signif(p1*p2,3)
p4 = signif((1-p1)*p2b,3)
...
In a deck of strange cards, the chance of drawing a `r image` is `r pi*100`%. If a `r image` is drawn, there is a `r pcgi*100`% chance that it is `r color`. If a card that is not a `r image` is drawn, there is a `r pcgni*100`% chance that it is `r color`. 

Now, someone draws a random card and reveals it is `r wrd1` `r color`. What is the chance the card is `r wrd2` a `r image`?
...
$$P(\text{"`r wrd2``r image`" given "`r wrd1``r color`"}) = \frac{`r p3`}{`r p3`+`r p4`} = `r p5` $$


Meta-information
============
extype: string
exsolution: $P(\text{"`r wrd2``r image`" given "`r wrd1``r color`"}) = `r p5` $
exname: marbles
extol: 0.01
