...
phat = round(p+z*se,3)
z = round((phat-p)/se,2)
Pr = round(1-2*pnorm(-z),4)
wrd = sample(c('sad','angry','special','greedy','bad','great','strong','happy','popular','cold'),1)
wrd2 = sample(c('within', 'closer than', 'between','inside'),1)
```

Question
========

In a very large population, `r p*100`% are `r wrd`. When a random sample of size `r n` is taken, what is the chance that the sample proportion of `r wrd` individuals is `r wrd2`  $\pm$ `r (phat-p)*100` percentage points from `r p*100`%? 
...
We are looking for a central area ("`r wrd2`  $\pm$ `r (phat-p)*100` percentage points from `r p*100`%").

```{r phat_sampling_central, echo = FALSE, results = "hide", fig.height = 3, fig.width = 6, fig.path = "", fig.cap = ""}
par(mar=c(5,3,1,1))
...
$$\text{Pr}\left(|\hat{P}-`r p`| < `r phat-p`\right) ~~=~~ \text{Pr}\left(|Z| < `r z`\right) ~~=~~ 2\cdot\Phi(`r z`)-1 ~~=~~ `r Pr` $$
...
extype: num
exsolution: `r Pr`
exname: phat sampling cental prob
extol: 0.01
